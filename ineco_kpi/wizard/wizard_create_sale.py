...
class inece_kpi_create_sale(osv.osv_memory):
    
    _name = "ineco.kpi.create_sale"
    _columns = {
    ...
    def create_wizard(self, cr, uid, ids, context=None):
        ...
        for sale_id in sale_user_ids:
            data_ids = self.pool.get('ineco.kpi.sale').search(cr, uid, 
            ...
            if not data_ids:
                ...
                self.pool.get('ineco.kpi.sale').create(cr, uid, new_data)
