...
class wizard_ineco_report_add(osv.osv_memory):
    ...
    def do_action(self, cr, uid, ids, context=None):
        ...
        if not report_ids:
            res = self.pool.get('ir.model.data').ir_set(cr, uid, 'action', 'client_print_multi', report.report_name, [report.model], 
                'ir.actions.report.xml,%d' % datas['ids'][0], isobject=True)
        else:
            raise osv.except_osv(_('Error!'), _("Can not add duplicate button!"))
        return self.write(cr, uid, ids, {'state':'get'}, context=context)

    _name = "wizard.ineco.report.add"
    _description = "Add Report Button"
